...
class StreamingServer(object):
    ...
    async def handle_connection_impl(
        ...
        while True:
            ...
            stream.accept_waveform(sampling_rate=self.sample_rate, waveform=samples)
        ...
        tail_padding = torch.rand(int(self.sample_rate * 0.3), dtype=torch.float32)
    ...
    async def recv_audio_samples(
        ...
        in 16 kHz or contains "EOS" meaning the end of utterance.
        ...
        with warnings.catch_warnings():
            ...
            if hasattr(torch, "frombuffer"):
                ...
                message = torch.frombuffer(message, dtype=torch.int16)
            else:
                message = np.frombuffer(message, dtype=np.int16)
                message = torch.from_numpy(message)

        message = message.to(torch.float32)
        message = message / 32768.0
        return message
